...
Eigen::Vector3f spcGenericCloud::getPoint(const int id) const
{
    float x,y,z;
    getPoint(id, x, y, z);
...
pcl::PointCloud<pcl::PointXYZ> spcGenericCloud::applyTransform(const Eigen::Transform<float, 3, Eigen::Affine, Eigen::AutoAlign> &T)
{
...
std::vector<float> spc::spcGenericCloud::getField(const std::string fieldname, std::vector<int> indices)
{
    ...
    {
        pcl::console::print_warn("[Error in generic_cloud] asked for field %s", fieldname.c_str());
        return out;
    }
