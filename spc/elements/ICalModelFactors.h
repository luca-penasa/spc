 ...
 */
class IntensityCalibratrionModelFactorsBased
    : public IntensityCalibrationModelBase
{
public:
    typedef spcSharedPtrMacro<IntensityCalibratrionModelFactorsBased> Ptr;
    typedef const spcSharedPtrMacro
        <IntensityCalibratrionModelFactorsBased> ConstPtr;
    ...
    /** you are responsable to pass the right number of pars here. No check will
     * be made
     */
    virtual void initParameters(const Eigen::VectorXf &init_pars
                                = Eigen::VectorXf())
    {
        ...
        if (init_pars.cols()
            == 1) // NOTE we will never have 1-dim parameters vecotr hopefully!
        {
            pcl::console::print_debug(
                "initializin parameters because passed are empty\n");

            this->initParametersInFactors();
            this->readParametersFromFactors();
        } else {
            pcl::console::print_debug(
                "initializing parameters with passed ones\n");
    ...
    void writeParametersToFactors(const Eigen::VectorXf &pars)
    {
        ...
        spcForEachMacro(CorrectionFactorBase::Ptr fac, factors_)
        {
            ...
            Eigen::VectorXf par
                = parameters_.segment(position_in_parameters, n_pars);
...
//!
//! \brief The JutzyModel class is special calibration model based on the paper
//Jutzi, 2009
...
public:
    ...
    JutzyModel(SamplesDB::ConstPtr db, const bool fixed_mul = true)
    {
        ...
        CorrectionFactorCosPowerAngle::Ptr ang_f(
            new CorrectionFactorCosPowerAngle(db));

        CorrectionFactorBase::Ptr mul_fixed
            = CorrectionFactorFixedMultiplier::Ptr(
                new CorrectionFactorFixedMultiplier(db));

        CorrectionFactorBase::Ptr mul_per_cloud
            = CorrectionFactorCloudDependentMultiplier::Ptr(
                new CorrectionFactorCloudDependentMultiplier(db));
