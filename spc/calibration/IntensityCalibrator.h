...
public:
    ...
    void setCalibrationDataDB(CalibrationDataDB::Ptr db)
    {
        assert(db);
        db_ = db ;
    ...
    CalibrationDataDB::Ptr getCalibrationDataDB() const
    {
        return db_;
    }
    ...
    // this functor returns simply the difference between the observed and predicted intensities
    struct FunctorSimple: FunctorBase<float>
    {
        ...
        int operator() (const Eigen::VectorXf &pars, Eigen::VectorXf &residuals) const
        {
    ...
    int optimize()
    {
        ...
        std::cout << "x that minimizes the function: " << std::endl << pars << std::endl;
