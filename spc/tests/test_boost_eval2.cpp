...
struct lazy_nfunc_
{
    ...
    template <typename F, typename A1>
    double operator()(F f, A1 object) const
    {
...
template <typename T>
T getDistance(const spc::CorePointData::Ptr core)
{
    return static_cast<T> (core->value<float>("distance"));
}

template <typename FPT, typename Iterator>
struct grammar: boost::spirit::qi::grammar< Iterator, FPT(), boost::spirit::ascii::space_type >
{
    ...
    struct constant_: boost::spirit::qi::symbols<typename std::iterator_traits<Iterator>::value_type,FPT>
    {
        constant_()
        {
            this->add
    ...
    //     /nullary functions
    struct nfunc_: boost::spirit::qi::symbols<typename std::iterator_traits<Iterator>::value_type, FPT (*)(const spc::CorePointData::Ptr)  >
    {
        ...
        {
            this->add
                    ("r" , static_cast<FPT (*)(const spc::CorePointData::Ptr)>( &getDistance<FPT> ))
                    ;
        }
    } nfunc;

    //     /nullary functions
    struct nfunc_: boost::spirit::qi::symbols<typename std::iterator_traits<Iterator>::value_type,  * FPT  >
    {
        ...
        {
            this->add
                    ("v" , static_cast<FPT (*)(const spc::CorePointData::Ptr)>( &getDistance<FPT> ))
                    ;
        }
    } vectors;
    ...
    struct bfunc_: boost::spirit::qi::symbols< typename std::iterator_traits<Iterator>::value_type, FPT (*)(FPT, FPT)>
    {
        bfunc_()
        {
            this->add
    ...
    boost::spirit::qi::rule< Iterator, FPT(), boost::spirit::ascii::space_type> expression, term, factor, primary;
    ...
    grammar() : grammar::base_type(expression)
    {
        ...
        expression =
                ...
                | (no_case[bfunc] >> '(' >> expression >> ','  >> expression >> ')')  [_val = lazy_bfunc(_1, _2, _3)]
                | no_case[constant] [_val = _1]
                | no_case[nfunc] [_val = lazy_nfunc(_1)]
                | '_' >>
                ;
                ...
                template <typename FPT, typename Iterator>
                bool parse(Iterator &iter, Iterator end, const grammar<FPT,Iterator> &g, FPT &result)
        {
