...
public:
    ...
    inline OutSeriesPtrT getOutput() const
    {
        return out_series_;
    }

    inline void setX(const vType &x)
    {
        x_ = x;
    ...
    inline void setY(const vType &y)
    {
        y_ = y;
        input_modified_ = true;
    }

    inline void setXY(const vType &x, const vType &y)
    {
        x_ = x;
        y_ = y;
        input_modified_ = true;
    ...
    void setBandwidth(const ScalarT bandwidth)
    {
        bandwidth_ = bandwidth;
    }

    void setStep(const ScalarT step)
    {
        step_ = step;
    }

    void setComputeVariance(const bool &compute_var = true)
    {
    ...
    vType getVar()
    {
        if (compute_var_ == false) {
            std::cout << "ERROR: you did not enabled variance computation at "
                         "compute() time \n variance vector is void"
                      << std::endl;
        }
        return variance_;
    }

    void setWeights(const vType &weights)
    {
        weights_ = weights;
    }

    void getExternalWeightsForIds(const idvType ids, vType &weights);

    void setUseWeights(const bool useit)
    {
        use_weights_ = useit;
    }
...
private: // methods
    ...
    void gaussian(const vType &values, vType &gaussian_values);
    ...
    int evaluateKS(const ScalarT &position, ScalarT &value, ScalarT &var);
};
