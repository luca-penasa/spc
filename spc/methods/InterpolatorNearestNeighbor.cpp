...
int InterpolatorNN::compute()
{

    if (is_input_ok_ == false) {
        std::cout << "You must set up input vectors first! Remember that both "
                     "must have same size." << std::endl;
        return -1;
    }
    int n = new_x_.size();
...
int InterpolatorNN::getNearestID(const float &value)
{
    int n_id = 0;
...
float InterpolatorNN::getNearestValue(const float &value)
{
