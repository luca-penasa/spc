...
int KernelSmoothing::compute()
{
    ...
    if (!out_series_) {
        out_series_
            = OutSeriesPtrT(new OutSeriesT(get_min(x_), get_max(x_), step_));
        std::cout << "time series wil be automaticcaly of full size"
                  << std::endl;
    }
                            ...
                            idvType &ids, vType &distances)
{
    ...
    flann::SearchParams search_params
        = flann::SearchParams(-1, epsilon, sorted);
...
int KernelSmoothing::evaluateKS(const ScalarT &position, ScalarT &value, ScalarT &var)
{
    ...
    if (use_weights_) {
        ...
        std::transform(weights.begin(), weights.end(), external_w.begin(),
                       weights.begin(), std::multiplies<ScalarT>());
    }

    vType vproduct; // element by element product TODO this should be done as
                    // above
