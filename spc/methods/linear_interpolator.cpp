...
////    std::cout << "you have " << upper_points_.size() << " upper points" << std::endl;
////    std::cout << "you have " << lower_points_.size() << " lower points" << std::endl;

//    int n = degree_ + 1; //number of points we will need to estimate a polynomials for estimating the out-of bound function
//    //get these points
//    auto ids_in_first_segment = segments_.at(0);
//    std::vector<T> values_first_segment (ids_in_first_segment.size());

//    //get the corresponding positions
//    std::transform(ids_in_first_segment.begin(), ids_in_first_segment.end(), values_first_segment.begin(), [&]( int id) {return this->new_x_[id];});

//    //now we need to find out the lower n points. sometimes the new_x vector could be not ordered
//    auto lower_n_ids = getFirstNSortingIds(values_first_segment, n);

//    std::vector<int> global_ids(lower_n_ids.size());
//    std::transform(lower_n_ids.begin(), lower_n_ids.end(), global_ids.begin(), [&](int id ){return ids_in_first_segment[id];});

//    std::vector<T> tmp_x(n), tmp_y(n);
...
//    std::vector <T> to_evaluate(lower_points_.size());
//    std::transform(lower_points_.begin(), lower_points_.end(), to_evaluate.begin(), [&](int id){return this->new_x_[id];});
