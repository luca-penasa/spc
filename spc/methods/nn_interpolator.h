 ...
 * performs nninterpolation. Class makes use of linear search for finding neighbors
 * 
 * \author Luca Penasa
 */
...
public:
	 ...
	 */
	void 
	setXY(const std::vector<float> &x, const std::vector<float> &y)
	{
		setX(x);
		setY(y);
		check_input_size();
	 ...
	 *  \param[in] new_x vector of x poisitions at which to evaluate the interpolator
	 */
	void 
	setNewX(const std::vector<float> &new_x)
	{
		new_x_ = new_x;
		n_out_ = new_x_.size();
	}
	...
	getNewY()
	{
		...
		{
			std::cout << "You need to compute output first! Use compute() method." << std::endl;
			return new_y_;
		}
 ...
 * \note size of x and y must be the same and != zero. Also a new_x array must be present and not null.
 */
	bool
	check_input_size()
