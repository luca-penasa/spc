                               ...
                               pcl::PointCloud<PointD> &outcloud)
{
    ...
    for (int i = 0; i < incloud.size(); ++i)
        outcloud.points[i].distance
            = pcl::euclideanDistance(incloud.points[i], this_origin);
}
...
template void computeDistanceFromOrigin(const pcl::PointCloud
                                        <pcl::PointXYZ> &incloud,
                                        pcl::PointCloud<PointD> &outcloud);

template float getAverageDistanceFromSensor(const pcl::PointCloud
                                            <pcl::PointXYZI> &cloud,
                                            const std::vector<int> ids);
