...
namespace spc
{
template <typename nType> class InterpolatorRBF
{
...
public:
    ...
    nType evaluateRbf(VectorT &point);
    ...
    void setInputPoints(MatrixT &points)
    {
    ...
    // Used for 1-d case
    void setInputPoints(const std::vector<nType> &points)
    {
        // we need to map the std::vector to an Eigen matrix type

        points_ = Eigen::Matrix
            <nType, Eigen::Dynamic, 1>::Map(&points[0], points.size());
        updateInputPointsInfo();
    }

    void updateInputPointsInfo()
    {
    ...
    void setInputValues(std::vector<nType> &values)
    {
        values_ = Eigen::Matrix
            <nType, Eigen::Dynamic, 1>::Map(&values[0], values.size());
    }
    ...
    void updateMatrixOfDistances();

    void updateMatrixOfCoefficients();
