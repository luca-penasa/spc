...
std::vector<ScalarT> *PointCloud2Reader::getScalarFieldAsStdVector(std::string
                                                                   &field_name)
{
    ...
    if (in_cloud_ != 0) // sensor_msgs cloud!
    {
        ...
        if (id == -1) {
            pcl::console::print_error(
                "Requested scalar field in pointcloud2reader not exists!");
            return nullptr; // return a null pointer
        }

        // now we get all the infos about this field
        pcl::PCLPointField field = in_cloud_->fields.at(id);
        ...
        } else // we MUST get a cast to make the data fit!
        {
            ...
            {
                pcl::console::print_error("something wrong in the pointcloud2 "
                                          "reader! - wrong sized of fields");
                return nullptr;
            }
        }
    ...
    } else if (in_cloud_cc_) // a ccPointCloud
    {
        ...
        pcl::console::print_error(
            "You should give an input for using the reader!");
        return 0;
    }
...
PointCloud2Reader::getScalarFieldAsStdVector(std::string &field_name);
