...
float
getValue(const unsigned int point_id, const string field_name, const pcl::PCLPointCloud2 & cloud, float &value, const unsigned int count_id=0)
{
    unsigned int point_step = cloud.point_step;
...
int main(int argc, char ** argv)
{
    ...
    BOOST_FOREACH (auto id, train_pcd_indices)
...
//    stream.precision (precision);
   ...
   pcl::PCLPointCloud2::Ptr features_cloud (new pcl::PCLPointCloud2);
    ...
    BOOST_FOREACH (auto name, train_pcd_fns) //for each class
    {
        ...
        float value;
        BOOST_FOREACH (auto id, indices) //for each index
        {
