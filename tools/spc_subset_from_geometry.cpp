...
void getNearestNeighbor(const PointXYZ &point,
						const KdTreeFLANN<PointXYZ> &flann, float &distance,
						int &index) {
  vector<int> indices;
...
void getCorrespondingIds(const PointCloud<PointXYZ>::Ptr reference_cloud,
...
int main(int argc, char **argv) {
  ...
  vector<int> pcd_files_indices = parse_file_extension_argument(
	  argc, argv, ".pcd"); // first is input second queries point

  string ids_file;
  parse_argument(argc, argv, "-id", ids_file);
  pcl::PCLPointCloud2::Ptr original_cloud(new pcl::PCLPointCloud2);
  pcl::io::loadPCDFile(argv[pcd_files_indices[0]],
					   *original_cloud); // load first cloud

  pcl::PCLPointCloud2 query_cloud;
  pcl::io::loadPCDFile(argv[pcd_files_indices[1]],
					   query_cloud); // load the second as gemetric query cloud
  ...
  if (ids_file.empty())
	getCorrespondingIds(original_cloud_pcl, *query_cloud_pcl, indices,
						distances);
  else {
	...
	std::transform(ids_cloud.begin(), ids_cloud.end(), indices.begin(),
				   [](PointId &point) { return point.id; });
  }
  ...
  //	const boost::shared_ptr<const pcl::PCLPointCloud2> original_cloud_ptr =
  //boost::make_shared<const pcl::PCLPointCloud2>(original_cloud);

  //	pcl::PCLPointCloud2::ConstPtr original_cloud_ptr =  original_cloud.
